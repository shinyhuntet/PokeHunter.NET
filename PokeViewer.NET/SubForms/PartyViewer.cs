...
namespace PokeViewer.NET.SubForms
{
    public partial class PartyViewer : Form
    {
        ...
        public PartyViewer(int gametype, ref ViewerState executor, (Color, Color) color)
        {
            InitializeComponent();
            Executor = executor;
            GameType = gametype;
        ...
        private void SanityCheck(PKM pk, int count)
        {
            ...
            string output = $"{(pk.ShinyXor == 0 ? "■ - " : pk.ShinyXor <= 16 ? "★ - " : "")}{(Species)pk.Species}{form}{gender}{Environment.NewLine}{(Nature)pk.Nature} Nature | Ability: {GameInfo.GetStrings(1).Ability[pk.Ability]} | IVs: {pk.IV_HP}/{pk.IV_ATK}/{pk.IV_DEF}/{pk.IV_SPA}/{pk.IV_SPD}/{pk.IV_SPE}{Environment.NewLine}EVs: {pk.EV_HP}/{pk.EV_ATK}/{pk.EV_DEF}/{pk.EV_SPA}/{pk.EV_SPD}/{pk.EV_SPE}{Environment.NewLine}{GameInfo.GetStrings(1).Move[pk.Move1]} - {pk.Move1_PP} | {GameInfo.GetStrings(1).Move[pk.Move2]} - {pk.Move2_PP}{Environment.NewLine}{GameInfo.GetStrings(1).Move[pk.Move3]} - {pk.Move3_PP} | {GameInfo.GetStrings(1).Move[pk.Move4]} - {pk.Move4_PP}{Environment.NewLine}{(pk.IsEgg ? "Hatch Cycle" : "Friendship")}: {pk.CurrentFriendship}";
                    ...
                    case (int)GameSelected.Scarlet or (int)GameSelected.Violet:
                        {
                            ...
                            var pointer = new long[] { 0x4763C98, 0x08, val, 0x30, 0x00 };// ver 3.0.1
