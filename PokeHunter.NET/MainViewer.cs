...
namespace PokeViewer.NET
{
    public partial class MainViewer : Form
    {
        ...
        private async Task Window_Loaded(CancellationToken token)
        {
            ...
            (string, string, string) strings = (string.Empty, string.Empty, string.Empty);
            switch (title)
            {
                case HOMEID:
                    {
            ...
            BeginInvoke((MethodInvoker)delegate ()
            {
                ...
                Languagelabel.Text = strings.Item3;
        ...
        private void ViewerControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (currentTab)
            {
                ...
                case "Egg 🥚":
                    selectedInt = 4; ViewerControl.Height = 975;
                    ViewerControl.Width = 840;
                    Height = 955;
                    Width = 830;
                    break;
                ...
                case "Misc 📓":
                    selectedInt = 7; ViewerControl.Height = 570;
                    ViewerControl.Width = 1260;
                    Height = 540;
                    Width = 1250;
                    break;
            }
            if (selectedInt is not 4 && selectedInt is not 7)
            {
                ViewerControl.Height = 582;
                ViewerControl.Width = 531;
                Height = 552;
                Width = 523;
            }
            ...
            switch (currentTab)
            {
                ...
                case "Box 📦": form = new BoxViewerMode(GameType, ref Executor, colors, trainerStrings) { TopLevel = false }; FormLoaded[2] = true; break;
                case "Party 👨‍👩‍👦‍👦": form = new PartyViewer(GameType, ref Executor, colors) { TopLevel = false }; FormLoaded[3] = true; break;
                case "Egg 🥚":
                    form = new Egg_Viewer(GameType, ref Executor, colors, Trainer) { TopLevel = false }; FormLoaded[4] = true; ViewerControl.Height = 975;
                    ViewerControl.Width = 840;
                    Height = 955;
                    Width = 830;
                    break;
                ...
                case "Misc 📓":
                    form = new MiscViewer(ref Executor, GameType, colors) { TopLevel = false }; FormLoaded[7] = true; ViewerControl.Height = 575;
                    ViewerControl.Width = 1260;
                    Height = 545;
                    Width = 1250;
                    break;
            }
            if (currentTab is not "Egg 🥚" && currentTab is not "Misc 📓")
            {
                ViewerControl.Height = 582;
                ViewerControl.Width = 531;
                Height = 552;
        ...
        public static (Color, Color) CheckForColors(bool mode)
        {
            if (mode is true)
        ...
        private void WideButton_Click(object sender, EventArgs e)
        {
            ...
            switch (GameType)
            {
                case (int)GameSelected.Scarlet or (int)GameSelected.Violet:
                    {
                        form = new WideViewerSV(GameType, ref Executor, colors, Trainer);
                        break;
                    }
                case (int)GameSelected.LegendsArceus:
                    {
        ...
        private void StaticButton_Click(object sender, EventArgs e)
        {
            ...
            switch (GameType)
            {
                case (int)GameSelected.Scarlet or (int)GameSelected.Violet:
                    {
                        form = new StaticViewer(GameType, ref Executor, colors, Trainer);
                        break;
                    }
                default:
                    {
        ...
        private void ScreenTrackBar_Scroll(object sender, EventArgs e)
        {
            ...
            {
                result1 =  MessageBox.Show("Change Screen State to OFF?", "Screen prompt", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                state = ScreenState.Off;
            }
            else
            {
                result1 =  MessageBox.Show("Change Screen State to ON?", "Screen prompt", MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                state = ScreenState.On;
            }
            if (result1 == DialogResult.Yes)
                SetScreen(state, CancellationToken.None).ConfigureAwait(false);
            else
            {
                if (ScreenTrackBar.Value == 1)
                    ScreenTrackBar.Value = 0;
                else
        ...
        #region TrainerSav
        // via SysBot.NET
        public async Task<(string, string, string)> GetFakeTrainerSAVSWSH(CancellationToken token)
        {
            ...
            return ($"Name: {Trainer.OT}", $"TID: {sav.TrainerTID7} ({sav.TID16})\nSID: {sav.TrainerSID7} ({sav.SID16})", $"Language: {(LanguageID)Trainer.Language}");
        }
        public async Task<(string, string, string)> GetFakeTrainerSAVSV(CancellationToken token)
        {
            var sav = new SAV9SV();
            var info = sav.MyStatus;
        ...
        public async Task<(string, string, string)> GetFakeTrainerSAVLA(CancellationToken token)
        {
            var sav = new SAV8LA();
            var info = sav.MyStatus;
        ...
        public async Task<(string, string, string)> GetFakeTrainerSAVBDSP(int type, CancellationToken token)
        {
            var sav = new SAV8BS();
            var info = sav.MyStatus;
            ...
            var lang = await Executor.SwitchConnection.PointerPeek(1, (type == (int)GameSelected.BrilliantDiamond ? Offsets.ConfigLanguagePointerBD : Offsets.ConfigLanguagePointerSP), token).ConfigureAwait(false);
            sav.Language = lang[0];
            Trainer = new()
            {
                OT = sav.OT,
        ...
        public (string, string, string) GetFakeTrainerSAVLGPE()
        {
            var sav = new SAV7b();
            Span<byte> bytes = sav.Blocks.Status.Data;
            byte[]? data = ReadLGEPData;
            data.CopyTo(bytes);
            Trainer = new()
            {
                OT = sav.OT,
        ...
        private byte[] ReadLGEPData => Executor.SwitchConnection.ReadBytesAsync(Offsets.TrainerDataLGPE, Offsets.TrainerSizeLGPE, CancellationToken.None).Result;
        ...
        private void EventRedeem_Click(object sender, EventArgs e)
        {
            ...
            switch(GameType)
            {
                case (int)GameSelected.Scarlet or (int)GameSelected.Violet: form = new EventCodeEntry(GameType, ref Executor, colors, Trainer); break;
                case (int)GameSelected.Sword or (int)GameSelected.Shield: form = new EventCodeEntrySWSH(GameType, ref Executor, colors, Trainer); break;
                default: return;
            };
            form.ShowDialog();
        }
        ...
        {
            var colors = CheckForColors(Settings.Default.DarkMode);
            ControllerView form = new(ref Executor, colors);
            form.ShowDialog();
        }
