        ...
        private void EnableAssets(List<Image> sprite, List<byte[]?> pos, List<string> count, List<string> strings, List<Image> spriteob, List<string> countob, List<string> stringsob, List<byte[]?> pos2)
        {
            ...
            for (int i = 0; i < spriteob.Count; i++)
            {
                switch (i)
                {
                    ...
                    case 5: EventSpr6.Image = spriteob[i]; EventCount6.Text = countob[i]; EventName6.Text = stringsob[i]; break;
                    case 6: EventSpr7.Image = spriteob[i]; EventCount7.Text = countob[i]; EventName7.Text = stringsob[i]; break;
                    case 7: EventSpr8.Image = spriteob[i]; EventCount8.Text = countob[i]; EventName8.Text = stringsob[i]; break;
                    case 8: EventSpr9.Image = spriteob[i]; EventCount9.Text = countob[i]; EventName9.Text = stringsob[i]; break;
                    case 9: EventSpr10.Image = spriteob[i]; EventCount10.Text = countob[i]; EventName10.Text = stringsob[i]; break;
                }
            }
        }
    }
